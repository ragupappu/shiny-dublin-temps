...
viztemps <- function (start_year,
  ...
  p <- ggplot(Present, aes(newDay, tmaxF)) +
  ...
  p <- p + geom_linerange(Present,
                          mapping=aes(x=newDay, ymin=tminF, ymax=tmaxF),
  ...
  if(start_year != 2014) {
    if(n2014highs != 0) {
      ...
      p <- p + geom_point(data=PresentHighs, aes(x=newDay, y=tmaxF), colour="firebrick3")
    ...
    if(n2014lows != 0) {
      ...
      p <- p + geom_point(data=PresentLows, aes(x=newDay, y=tminF), colour="blue3")
